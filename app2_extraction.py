...
def read_input_files(input_file_sentences, input_file_app2):
    ...
    df_sentences = (
        ...
        .drop(['subject', 'subject_gen'], axis=1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Extracts qualitative traits"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        'input_file_app2', 
        help="Path to the input text file containing the appendix2"
    )
    parser.add_argument(
        'prompt_style',
        choices=['zeroshot', 'fewshot', 'cot', 'cot-fewshot'], 
        help=(
    ...
    for taxon_name in df_sentences.taxon_name.unique():
        ...
        for subject in df_app2.subject.unique():
            # Sentences about a specific species and subject are joined together into a paragraph
            subject_para = " ".join(
                df_sentences[
                    (df_sentences.subject == subject) & (df_sentences.taxon_name == taxon_name)
                ]['sentence'].to_list()
            )
            ...
            for batch in batches:
                ...
                # textwrap.dedent strips out the indenting spaces in the multiline text string
                ...
                prompt = prompt_outline.format(
                    subject_para=subject_para,
                    appendix_2_subject_batch=appendix_2_subject_batch,
                )
