...
for taxon_name in df_sentences.taxon_name.to_list():
    print(taxon_name)
...
# prompt = f"""
# Using only the files provided below, generate a csv which converts these sentences into a tabular datafile with quantitative data about the traits. These files are taken from a botanical monograph. The first column heading should be 'species_name', the following column headings should be the abbreviations from appendix_1. The categories in sentences_cat correspond to the subjects in appendix_1.
# Give the output as a csv not a python script. Do not fabricate data and ensure the values correspond to the correct abbreviation, if values for traits are not recored, leave blank. Respond only with a csv.
# {appendix_1}
# {sentences_cat}
# """

# Output and convert to a csv file


with open('output.csv', 'w', newline='') as csvfile:
    csvfile.write(sentence_dict)
